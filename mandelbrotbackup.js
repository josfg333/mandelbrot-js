...
function qrun(){
    ...
    function inset(m,n){
        ...
        for(h=0; h<limit; h++){
            ...
            oldworkingm=workingm;
            if(Math.sqrt((workingm**2)+(workingn**2)+(workingj**2)+(workingk**2))>2){
                return(h);
            }
        }
        return(h);
    ...
    for(x=0; x<width; x++){
        for(y=0; y<height; y++){
            if(width<height){
                var a=(((x/(width/4))-2)/zoom)+xcor;
                var b=((2-(y/(width/4))+((2*(height-width))/width))/zoom)+ycor;
            }else{
                var a=(((x/(height/4))-(2+(2*(width-height))/height))/zoom)+xcor;
                var b=((2-(y/(height/4)))/zoom)+ycor;
            }
            var value=inset(a,b);
            list.push(value);
        }
